...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        internal KdTree(Simulator simulator)
        {
         ...
         */
        internal void buildAgentTree(bool force)
        {
            if (agents_ == null || agents_.Length != _simulator.agents_.Count || force)
            {
                agents_ = new Agent[_simulator.agents_.Count];

                for (int i = 0; i < agents_.Length; ++i)
                {
                    agents_[i] = _simulator.agents_[i];
                }
        ...
        internal void buildObstacleTree()
        {
            ...
            IList<Obstacle> obstacles = new List<Obstacle>(_simulator.obstacles_.Count);

            for (int i = 0; i < _simulator.obstacles_.Count; ++i)
            {
                obstacles.Add(_simulator.obstacles_[i]);
            }
...

            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    else
                    {
                        ...
                        newObstacle.id_ = _simulator.obstacles_.Count;

                        _simulator.obstacles_.Add(newObstacle);
